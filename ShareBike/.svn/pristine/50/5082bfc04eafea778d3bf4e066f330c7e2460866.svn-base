...
public class SocketService extends Service {
    final SocketService self = this;
    private boolean isConnected = true;

    @Override
    public void onCreate() {
        super.onCreate();
        Logger.e("socket---->oncreate");

    }

    @Override
    public void onStart(Intent intent, int startId) {
        Logger.e("socket---->onstart1");
        //  connect();
        Logger.e("socket---->onstart2");
    }

    @Override
    public int onStartCommand(Intent intent, int flags, int startId) {
        Logger.e("socket---->onstart3");
...
//            __i__setupReadManuallyForReceiver(this.localSocketClient);
            ...
            this.localSocketClient.registerSocketClientDelegate(new SocketClientDelegate() {
                ...
                public void onConnected(SocketClient client) {
                    Logger.e("onConnected", "SocketClient: onConnected");
                    if (isConnected) {
                        send();
                    }
                    isConnected = false;
                ...
                public void onDisconnected(final SocketClient client) {
                    Logger.e("onDisconnected", "SocketClient: onDisconnected");
                ...
                public void onResponse(final SocketClient client, @NonNull SocketResponsePacket responsePacket) {
                    Log.e("onResponse", "SocketClient: onResponse:" + "走了");
            ...
            this.localSocketClient.registerSocketClientSendingDelegate(new SocketClientSendingDelegate() {
                ...
                public void onSendPacketBegin(SocketClient client, SocketPacket packet) {
                    Logger.e("onSend", "SocketClient: onSendPacketBegin: " + packet.hashCode() + "   " + Arrays.toString(packet.getData()));
                }

                @Override
                public void onSendPacketCancel(SocketClient client, SocketPacket packet) {
                    Logger.e("onSend", "SocketClient: onSendPacketCancel: " + packet.hashCode());
                }

                @Override
                public void onSendingPacketInProgress(SocketClient client, SocketPacket packet, float progress, int sendedLength) {
                    Logger.e("onSend", "SocketClient: onSendingPacketInProgress: " + packet.hashCode() + " : " + progress + " : " + sendedLength);
                }

                @Override
                public void onSendPacketEnd(SocketClient client, SocketPacket packet) {
                    Logger.e("onSend", "SocketClient: onSendPacketEnd: " + packet.hashCode());
                }
            });
            this.localSocketClient.registerSocketClientReceiveDelegate(new SocketClientReceivingDelegate() {
                ...
                public void onReceivePacketBegin(SocketClient client, SocketResponsePacket packet) {
                    Logger.e("onReceive", "SocketClient: onReceivePacketBegin: " + packet.hashCode());
                }

                @Override
                public void onReceivePacketEnd(SocketClient client, SocketResponsePacket packet) {
                    Logger.e("onReceive", "SocketClient: onReceivePacketEnd: " + packet.hashCode());
                }

                @Override
                public void onReceivePacketCancel(SocketClient client, SocketResponsePacket packet) {
                    Logger.e("onReceive", "SocketClient: onReceivePacketCancel: " + packet.hashCode());
                }
                ...
                public void onReceivingPacketInProgress(SocketClient client, SocketResponsePacket packet, float progress, int receivedLength) {
                    Logger.e("onReceive", "SocketClient: onReceivingPacketInProgress: " + packet.hashCode() + " : " + progress + " : " + receivedLength);
                }
            });
        }
...
//                ToastUtils.showToast(resultObject.getString("msg"));
    ...
    private void responseEvent(int type) {
        MessageEventInt eventInt = new MessageEventInt(type);
        eventInt.setType(type);
        EventBus.getDefault().post(eventInt);
    }
    ...
    private void __i__setupAddress(SocketClient socketClient) {
        ...
        //  socketClient.getAddress().setRemoteIP("192.168.0.177");//测试
        socketClient.getAddress().setRemoteIP("112.124.97.122");//正式

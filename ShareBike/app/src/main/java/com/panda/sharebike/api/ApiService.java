...
//    @GET("http://gank.io/api/data/福利/10/1")
    ...
    @FormUrlEncoded
    @POST(N.KEY_RENT)
    Observable<HttpResult<KbData>> getRent(@Field("token") String token, @Field("bikeId") String bikeld, @Field("longtitude") String longtitude, @Field("latitude") String latitude, @Field("maxDistance") double maxDistance);
    ...
    Observable<HttpResult<LoginDataBean>> getState(@Query("token") String token);

    //行程详情
    @FormUrlEncoded
    @POST(N.GET_GUIDE)
    Observable<HttpResult<GuideBean>> getGuide(@Field("token") String token, @Field("guideType") String guideType);

    //行程详情
    @FormUrlEncoded
    @POST(N.GET_VERSION)
    Observable<HttpResult<VersionBean>> getVersion(@Field("token") String token, @Field("appType") String appType);

    //锁
    @FormUrlEncoded
    @POST(N.GET_LOCK)
    Observable<HttpResult<Bock>> getLock(@Field("token") String token, @Field("bikeId") String bikeId);
}

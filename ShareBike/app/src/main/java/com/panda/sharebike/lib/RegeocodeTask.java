...
public class RegeocodeTask implements OnGeocodeSearchListener {
    ...
    private static final int TYPE_1 = 1;//1是自然定位
    private static final int TYPE_2 = 2;//2是计算车坐标
    private int type;
    ...
    public void search(double latitude, double longitude, int type) {
        this.type = type;
    ...
    public void onRegeocodeSearched(RegeocodeResult regeocodeReult,
                                    int resultCode) {
        if (resultCode == AMapException.CODE_AMAP_SUCCESS && type == TYPE_1) {
        ...
        if (resultCode == AMapException.CODE_AMAP_SUCCESS && type == TYPE_2) {
            if (regeocodeReult != null
                    && regeocodeReult.getRegeocodeAddress() != null
                    && mOnLocationGetListener != null) {
                String address = regeocodeReult.getRegeocodeAddress()
                ...
                entity.address = address;
                entity.city = city;
                mOnLocationGetListener.onRegecodeGetBike(entity);

...
public class SocketServices extends Service {
    ...
    private void initSocket() {
        ...
        socketClient.getAddress().setRemoteIP("47.97.170.136");
...
//                }.start();
                ...
                if (!userDisconnect) {
                    //每次间隔一分钟执行一次
                    handler.postDelayed(rideTimeTask, 1000 * 30);
                } else {
                    handler.removeCallbacks(rideTimeTask);
                }
            }
    ...
    private final Runnable rideTimeTask = new Runnable() {
        @Override
        public void run() {
            // 首先获得一个消息结
//            if (!userDisconnect) {
            ...
            if (resultObject.getInt("code") == 200) {
                if (resultObject.has("data")) {
                    ...
                    if (dataObj.has("type")) {
                        ...
                        if(dataObj.has("orderId")&&dataObj.getString("orderId")!=null) {
                            responseEvent(dataObj.getInt("type"),dataObj.getString("orderId"));
                        }else {
...
//                ToastUtils.showToast(resultObject.getString("msg"));
    ...
    private void responseEvent(int type,String orderId) {

        EventBus.getDefault().post(new MessageEventInt(type,orderId));
    }

...
//        }
    ...
    private void addPolylinessoild(LatLonPoint latLngA, LatLonPoint latLngD) {
        LatLng A = new LatLng(latLngA.getLatitude(), latLngA.getLongitude());
        LatLng D = new LatLng(latLngD.getLatitude(), latLngD.getLongitude());
        if (lineBitmap == null) {
                ...
                .subscribe(new Nsubscriber<HttpResult<DetailBean>>(new SubscriberListener<HttpResult<DetailBean>>() {
                    ...
                    public void onSuccess(HttpResult<DetailBean> model) {
                        if (model.isOk()) {
                            ...
                            mEndPoint = new LatLonPoint(Lat_D, Lon_D);
                            setfromandtoMarker();
                            addPolylinessoild(mStartPoint, mEndPoint);
                        }
                    }
